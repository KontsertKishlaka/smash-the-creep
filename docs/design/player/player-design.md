...
flowchart LR
  ...
  D --> E(["Export: .glb"])
  E --> F(["Import в Godot"])
  F --> G(["Добавление к сцене Игрока (player.tscn)"])
  G --> H(["State Machine / AnimationTree"])
```

> 🎨 Все анимации должны быть протестированы в Godot через `AnimationTree` и привязаны к состояниям игрока (`idle`, `run`, `attack`, `take_damage`, `death` и т.д.).
...
| ---------------------------------- | -------------------------------------------------------------- | ---------------------------------------- |
| **MUST HAVE**                      | Создать базовую low-poly модель девушки (humanoid)             | Без брони, оптимизированная для анимации |
...
| **WON’T HAVE (на текущий спринт)** | Полная кастомизация одежды и брони                             | Перенесено на фазу 2                     |
